...
# -------------------------------------------------------------------------
start_month <- c(2004, 1)   # 2004‑01
end_month   <- c(2019, 12)   # 2019‑12
...
shocks <- load_data(
  file.path("Data","Shocks","Shocks_m.rds"),
  "all_3_shocks.rds not found"
) %>%
  filter(Date >= start_date, Date <= end_date)

pureMP_m <- ts(shocks$Target_m,
               start     = start_month,
               frequency = 12)
...
out <-   file.path("Data","Interpolated data","control_var_m_sample.rds")
saveRDS(control, file = out)
...
Bundes_yield <- read_excel(
  file.path("Data","Additional","Generic Bundesbank yield.xlsx")
) %>%
  filter(Date >= start_date, Date <= end_date)
...
shocks_data <- cbind(
  ...
  Bund_2Y_m   = as.numeric(Bund_2Y_m),
...
# -------------------------------------------------------------------------
Bund_2Y_m_hat <- fitted.values(FirstStage)
saveRDS(Bund_2Y_m_hat, file = file.path("Data","LP-IV","1.stage_instrument_m.rds"))
...
# OBS: We lose the first 4 quarters, as we use 12 lags in the 1.stage.
shock_var_q_ts <- create_quarterly_ts(Bund_2Y_m_hat)

saveRDS(shock_var_q_ts, file = file.path("Data","LP-IV","1.stage_instrument_q.rds"))
cat("\nQuarterly instrument saved to", file.path("Data","1.stage_instrument_full_ts.rds"), "\n")
...
# === Plot 1: Actual vs Fitted Bund 2Y
plot_actual_fitted <- function() {
  plot(shocks_ts[, "Bund_2Y_m"], type = "l", col = burgundy_trans,
       main = "Actual vs. Fitted Bund 2Y",
       xlab = "Time", ylab = "Bund 2Y Yield (basis points)")
  lines(Bund_2Y_m_hat, col = burgundy, lwd = 2)
...
plot_shock_fitted <- function() {
  ...
  # (b) Plot the fitted 2Y Bund instrument
  plot(Bund_2Y_m_hat, type = "l",
       main = "(b) Fitted Bund 2Y Instrument",
...
plot_quarterly <- function() {
  plot(shock_var_q_ts,
       main = "Quarterly Fitted Bund 2Y Instrument",
       xlab = "Time", ylab = "Instrument Value (Fitted Bund 2Y)",
       col = burgundy, lwd = 2)
}
...
save_plot(file.path(plot_dir, "Actual_vs_Fitted_Bund_2Y.png"), plot_actual_fitted)
