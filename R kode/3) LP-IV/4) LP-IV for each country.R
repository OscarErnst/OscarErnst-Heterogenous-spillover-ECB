...
# 1. Load data + shock
# ─────────────────────────────────────────────────────────────────────────────
full_data <- readRDS("Data/LP-IV/input_data_q.rds")
shock_all <- readRDS("Data/LP-IV/shocks_q.rds")
...
plot_all_countries <- function(var, out_file) {
  ...
  for (ctry in countries) {
    ...
    df <- na.omit(d_ctry %>% dplyr::select(d_rGDP, d_HICP, bund_yield, shock))
    if (nrow(df) == 0) {
...
plot_all_countries(
  var      = "d_rGDP",
  out_file = "Graphs/LP-IV/Impulse Responses/All_Countries_d_rGDP.png"
)

plot_all_countries(
  var      = "d_HICP",
  out_file = "Graphs/LP-IV/Impulse Responses/All_Countries_d_HICP.png"
)
