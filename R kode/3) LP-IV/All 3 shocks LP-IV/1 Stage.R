...
shocks <- load_data(file.path("Data", "Shocks", "all_3_shocks.rds"),
                      "Target Factor Shock file not found!")


pureMP_m <- ts(shocks$pureMP_m, start = start_month, end = end_month, frequency = 12)
Path_m <- ts(shocks$Path_m, start = start_month, end = end_month, frequency = 12)
QE_m <- ts(shocks$QE_m, start = start_month, end = end_month, frequency = 12)
...
Bund_3M_m <- ts(Bundes_yield[[4]], start = start_month, frequency = 12)
...
# -------------------------------------------------------------------------
FirstStage <- dynlm(
  Bund_3M_m ~ pureMP_m + Path_m + QE_m +
...
# Joint F-test for contemporaneous instruments (pureMP_m, Path_m, QE_m)
joint_test <- linearHypothesis(
  FirstStage,
  c("pureMP_m = 0", "Path_m = 0", "QE_m = 0")
)

cat("\nJoint F-test Results for all 3 shocks:\n")
print(joint_test)
...
saveRDS(shock_var_q_ts, file = file.path(lpiv_dir, "Tamas_instrument.rds"))
